...
class CNN(nn.Module):
    def __init__(self,out_channel=4, k_h=1, k_w=2, stride=1, **kwargs):
        super(CNN, self).__init__()
        print('output_channel:', out_channel, '-----k_h:', k_h, '----k_w:',k_w)
        self.l1=nn.Conv2d(1, 2, (k_h,k_w), stride=stride, padding=0)
        self.nl1=nn.ReLU(True)
        self.l2=nn.Conv2d(2, out_channel, (k_h,k_w), stride=1, padding=0)
        self.nl2=nn.ReLU(True)
        self.l3=nn.Conv2d(out_channel, 1, (1,1), stride=1, padding=0)
...
class RandInput:
    def __init__(self):
        ...
        self.model_name = 'TwoLayer_'+self.cfg.arch + '_H' + str(self.cfg.height) + '_W' + str(self.cfg.width) + '_KH' + str(self.cfg.k_h)
        ...
        #self.logger = ext.logger.setting('log.txt', self.result_path)
        if self.cfg.arch == 'CNN':
            ...
            input_dim= self.cfg.height * self.cfg.width
            out_dim=(self.cfg.height-self.cfg.k_h+1)*(self.cfg.width - self.cfg.k_w +1) * self.cfg.out_channel
            self.model = MLP(input_dim=input_dim, out_dim=out_dim)
        else:
            self.model = LeNet()
       ...
       # self.logger('==> model [{}]: {}'.format(self.model_name, self.model))
        ...
        self.interFeature=[]
        #self.sampler=torch.distributions.Normal(0, 5)
        #self.sampler=torch.distributions.Uniform(-10, 10)
        return
    ...
    def add_arguments(self):
        ...
        parser.add_argument('-width', '--width', type=int, default=4)
        ...
        parser.add_argument('-std', '--std', type=int, default=11)
    ...
    def train_CNN(self, sample_number=50000, std=10, batch_size=1000, channels=1, width=2, height=2):
        ...
        self.model.l3.register_forward_hook(hook=self.hook_in_forward)
        for s in range(sample_number):
            ...
            with torch.no_grad():
                ...
                feature_data = torch.cat((feature_data, self.interFeature[1][0].view(batch_size,-1).data), 1)
